...
There are two main functions which are useful in a multivariate time series context. The first is ```FaBiSearch()``` which is used for change point estimation, and the second is ```estimate_network()``` which is used for estimating an adjacency matrix for network estimation from a stationary multivariate time series. We also include a ```plot_network()``` function which plots the estimated stationary network according to cortical atlas of @Gordon2016.
...
We utilize non-negative matrix factorization (NMF) to model the multivariate time series as a factor of lower dimensional projections, $W$ and $H$.

```{r echo=FALSE, out.width='70%', fig.align = 'center'}
knitr::include_graphics('NMFgeneral.png')
```
...
* ```data```: Multivariate time series, $Y$, to be analyzed, should be in a matrix format with time points in rows and variables in columns. Since NMF is being used to model the time series, all entries in the input matrix must be non-negative. ```FaBiSearch()``` can also sequentially compute change points across multiple subjects. If the dataset contains multiple subjects, the subjects must be concatenated end to end by rows, that is, after the first subject's matrix ends, the next row is the second subject's first time point, and so on and so forth.
...
The ```plot_network()``` function uses a @Gordon2016 atlas defined adjacency matrix and returns a 3D plot of the estimated stationary network of this adjacency matrix. Nodes are colored by membership in one of the 13 @Gordon2016 defined communities. Users can choose to plot only specific communities, and can also specify colors for these communities.

### Inputs

* ```adj.matrix```: 333 by 333 adjacency matrix, using the @Gordon2016 atlas to define nodes
* ```communities```: Vector of communities from the Gordon atlas to include in the plot (Any combination of "Default", "SMhand", "SMmouth", "Visual", "FrontoParietal", "Auditory", "None", "CinguloParietal", "RetrosplenialTemporal", "CinguloOperc", "VentralAttn", "Salience", "DorsalAttn)
* ```colors```: Vector of hex codes for node colors to distinguish each community - by default, each community is given a predefined color

### Output

A 3D plot of the estimated stationary network from the adjacency matrix. This plot includes a 3D "frame" of the cortex, edges colored in black, nodes colored based on community membership, and a legend related node colors to community membership. Please see the example below of a plot generated by this function:

```{r echo=FALSE, out.width='70%', fig.align = 'center'}
knitr::include_graphics('brainplotexample.png')
```
